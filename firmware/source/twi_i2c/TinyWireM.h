/*
  ...
  Thanks to 'jkl' for the gcc version of Atmel's i2c_Master code
  http://www.cs.cmu.edu/~dst/ARTSI/Create/PC%20Comm/
  I added Atmel's original Device dependant defines section back into i2c_Master.h
 ...
 USAGE is modeled after the standard Wire library . . .
  ...
  To Send:
	...
	someByte = TinyWireM.endTransmission(){          // actually send the bytes in the buffer
	                                                 // returns (optional) 0 = sucess or see i2c_Master.h for error codes
  To Receive:
	someByte = TinyWireM.requestFrom(uint8_t slaveAddr, uint8_t numBytes){      // reads 'numBytes' from slave's address
	                                                 // (usage optional) returns 0= success or see i2c_Master.h for error codes
...
#define USI_BUF_SIZE    16              // bytes in message buffer
	...
	uint8_t USI_BufIdx ;                    // current number of bytes in the send buff
	uint8_t USI_LastRead ;                  // number of bytes read so far
	uint8_t USI_BytesAvail ;                // number of bytes requested but not read
